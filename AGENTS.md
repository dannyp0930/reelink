...
- Use `impeccable` for UI design and refinement. Read `.agents/skills/impeccable/SKILL.md`; use `shape` before substantial screen changes, `critique` for UX review, and scoped `audit`/`polish` for implementation checks. A copy-only change does not require every command. Preserve the user's visual references and Ponytail's scope limits; do not add effects, assets, dependencies, or redesigns merely to satisfy aesthetic preferences.
- Resolve Impeccable's launcher from the repository root, then run UI context/design commands with `frontend` as cwd so this monorepo's backend is not selected as a design target. Do not invent `PRODUCT.md` or `DESIGN.md` content during tool installation; establish them from the approved brief during design work.
- For Reelink's record screens, benchmark Repov from actual reference screens before choosing a visual direction. Design list and calendar as alternate views of the same records with shared detail/edit flows; do not reduce the product plan to a list-only CRUD interface. This does not require implementing both views in every task.
- Impeccable supplements, not replaces, `web-design-guidelines` and Playwright verification. Review findings against product evidence; do not suppress rules just to obtain a clean report. If automatic hooks are unavailable or unapproved, run the scoped detector manually and distinguish that result from an automatic hook run.
...
- When scopes overlap, apply them in this order: scope with `ponytail`, use `superpowers` only if task complexity warrants it, shape/refine UI with `impeccable`, apply relevant domain and security checks plus final `web-design-guidelines` review, verify UI with `playwright-cli`, polish Korean documentation with `humanize-korean`, then report with `caveman`.
...
- Before continuing viewing UI work, read `docs/viewing-ui-plan.md`, `docs/project-status.md`, and `frontend/PRODUCT.md`. The UI plan is the approved delivery sequence; project status is completion evidence. Do not mark planned behavior as implemented or repeat settled product questions.
