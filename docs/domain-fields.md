...
- 상태: migration과 실제 DB 검증 완료. TMDB 영화 검색·선택 API, 본인 관람 CRUD API와 로그인 화면 구현. 영화 검색·관람 입력 화면과 굿즈 API는 미구현
...
TMDB 선택 API는 서버가 조회한 영화 정보로 내부 `Movie`를 생성하고 외부 ID를 연결한다. 재선택·동시 선택은 기존 연결을 재사용하며 제목으로 합치지 않는다. 입력·응답 계약은 `docs/movies-api.md`에 있다.
...
| 필드 | 타입 | 필수 | 규칙 |
| --- | --- | --- | --- |
| `userId` | UUID | 예 | 세션 사용자로 결정. 조회·수정·삭제도 본인 기록으로 제한 |
| `movieId` | UUID | 예 | `Movie` 참조 |
| `watchedOn` | date | 예 | API가 실제 존재하는 `YYYY-MM-DD` 날짜를 검증하고 UTC 자정으로 저장. 연도 `0001~9999` 허용 |
| `cinemaId` | UUID | 아니오 | `Cinema` 참조 |
| `auditorium` | string | 아니오 | 상영관 이름. 최대 100자 |
| `ratingHalfStars` | smallint | 아니오 | `0~10` 정수, 표시 평점은 `/ 2`. `null`은 미평가 |
| `note` | string | 아니오 | 개인 관람 메모. 최대 5,000자 |

- 입력 평점은 `0~5점`, `0.5점` 단위다. 작성·수정 API에서 `ratingToHalfStars`로 숫자·범위·간격을 검사하며 문자열 변환이나 반올림은 하지 않는다.
- Prisma `7.10.0`의 정수 입력이 소수를 잘라 저장하는 동작을 DB 테스트에서 확인했다. SMALLINT와 범위 CHECK만으로 원래 입력의 간격을 검증할 수 없다.
- 평점·날짜별 조회를 위해 `(userId, ratingHalfStars)`, `(userId, watchedOn)` 인덱스를 둔다. 목록·상세 조회 API에서 세션 사용자의 기록만 반환한다. 평점 필터는 후속 작업이다.
...
- 굿즈 제보·관측이 연결된 사용자는 삭제를 거부해 근거를 보존한다. 계정 삭제·익명화 API는 미구현이며 현재 정책은 `docs/auth-setup.md`에 기록했다.

### 관람 API 입력·응답

- 브라우저는 Next 프록시의 `/api/viewings`를 사용한다. Backend 직접 경로는 `/viewings`다.
- `POST /api/viewings`: `movieId`, `watchedOn` 필수. 선택 필드는 `cinemaId`, `auditorium`, `rating`, `note`다. 성공 시 `201`과 생성된 기록을 반환한다.
- `PATCH /api/viewings/:id`: 같은 입력 필드 중 하나 이상을 받는다. 생략한 필드는 유지한다. 선택 필드의 `null`은 값 비우기이며 `movieId`, `watchedOn`에는 허용하지 않는다.
- `DELETE /api/viewings/:id`: 해당 관람 기록만 삭제하고 `204`를 반환한다. 영화·극장과 다른 관람 회차는 남는다. 이미 삭제했거나 타인 기록이면 `404`다.
- 응답은 기존 조회 형식과 같다. `ratingHalfStars`는 반점 단위 정수, `watchedOn`은 UTC 자정 ISO 문자열이며 `movie`를 포함한다. 화면은 날짜를 사용자 시간대로 변환하지 말고 날짜 부분을 사용한다.
- `userId`, `id`, `ratingHalfStars`, 관계 객체 등 허용 목록 밖의 요청 필드는 `400`으로 거부한다. 빈 PATCH, 잘못된 날짜·UUID·평점, 존재하지 않는 영화·극장도 `400`이다.
- 문자열 길이는 JavaScript UTF-16 코드 단위로 검사한다. NUL 문자는 거부하고 빈 문자열은 그대로 보존한다. 문자열 입력은 HTML로 신뢰하지 않으며 화면에서 텍스트로 표시한다.
- 인증·`Origin` 검사는 기존 전역 가드를 사용한다. 수정·삭제의 DB 조건에 세션 `userId`를 포함한다. 여러 창의 정상 수정은 마지막 쓰기를 반영하며 버전 충돌 감지는 아직 없다.
