...
- `MovieViewing`에 선택 시각과 `THEATER`·`STREAMING`·`OTHER` 유형, 상영 포맷·OTT 플랫폼·기타 상세 필드를 추가했다. 기존 cinemaId 기록만 THEATER로 이관하고 나머지는 추정하지 않는다. 유형 변경은 맞지 않는 상세값을 같은 요청에서 비우며 시각 미입력과 `00:00`을 구분한다.
- 테스트 DB에 migration을 적용했다. `pnpm --dir backend test:e2e viewings` 72개, `pnpm --dir backend test:e2e viewing-migration` 1개, `pnpm --dir backend lint`, `pnpm --dir backend build`, `git diff --check` 통과. 실제 개발 DB에는 migration을 적용하지 않았고 Backend는 build 검증을 위해 중단했다.

2026-09-11 관람 유형 계획 조정:

- 사용자가 제공한 `관람 방식 설계` 대화와 관람 연월일·시각 확인을 반영해 UI 계획·제품 맥락·현황을 갱신했다. 추가 범위는 C1~C3·D·E에 배치했고 전 단계 Astra `high` 운영 계획을 유지한다. 통계·지도·OTT 계정 연동은 추가하지 않는다.
- 이번 턴은 문서만 변경했다. `git diff --check`를 확인했으며 앱·DB·migration·외부 서비스는 변경하지 않았다. 아래 테스트 결과는 이전 구현 턴의 근거이고 이번 턴에 재실행하지 않았다.

2026-09-11 별점 UI 간소화:

- 사용자 요청으로 별 아래 점수·미평가·0점 버튼을 제거했다. 선택한 별 재클릭·Delete 키는 미평가로 해제한다. 작성 UI는 0.5~5점과 미평가를 지원하며 기존 0점 데이터·API는 유지한다. 별 색상은 기존 흑백 테마를 유지했고 최종 디자인 색상은 미확정이다.
- `scripts/check-viewing-ui.cjs`: 1280×844·390×844에서 별 선택·재클릭·합성 터치·키보드 해제·null 저장과 기존 관람 흐름 통과. `pnpm --dir frontend lint`, `pnpm --dir frontend build` 통과. web-design-guidelines 변경 범위 검토와 Impeccable 수동 detector `[]` 확인. 자동 hook 검증은 아니다.

2026-09-11 관람 작성 검증·별점 입력:

- 평점 select를 별 선택으로 바꿨다. 별의 왼쪽·오른쪽 절반으로 0.5점 단위를 선택하며 0점과 미평가는 별도 항목으로 유지한다. native radio로 키보드 이동과 저장 중 잠금을 지원하고 기존 API 계약은 바꾸지 않았다.
- `scripts/check-viewing-ui.cjs`의 극장 locator와 이탈 확인창 처리를 수정했다. 1280×844·390×844에서 0~5점 전체 반점 값·미평가, 새 기록 초기화, 키보드 선택, 390px Chromium 합성 터치를 확인했다. 필수 날짜, 선택 항목 null, 저장 실패·401 입력 유지, 중복 제출 차단, 응답 유실 후 목록 확인, 극장 목록 실패·재시도·빈 목록, 비로그인 화면도 통과했다.
- `scripts/check-search-debounce.cjs`, `scripts/check-search-posters.cjs` 회귀 테스트와 `pnpm --dir frontend lint`, `pnpm --dir frontend build`, `git diff --check`를 통과했다. UI 테스트는 API·이미지 응답을 대체했으며 개인 DB에는 기록을 만들지 않았다. 의도한 HTTP·네트워크 실패 외에 예상하지 않은 콘솔·런타임 오류는 없었다.
- Impeccable 수동 detector는 `[]`를 반환했다. 밝은·어두운 별점 화면과 web-design-guidelines를 점검했다. 반별 터치 영역은 390px에서 24×48px로 44×44px 권장보다 좁다. 합성 터치는 통과했지만 실기기 편의성은 후속 확인 사항이다. 자동 hook 실행이나 전체 디자인 감사 완료를 의미하지 않는다.
- Frontend 3002·Backend 3001·DB를 실행했고 Frontend·세션 API `200`을 확인했다. 검증 브라우저는 종료했다. Backend 코드·DB 스키마·외부 서비스는 변경하지 않았으며 커밋·푸시는 하지 않았다.
